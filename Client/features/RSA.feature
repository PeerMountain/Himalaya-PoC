Feature: RSA Module
  ...
  Scenario Outline: Signature
    ...
    And I sign the message hash with RSA
    And convert the signature from long to bytes
    And I encode the signature bytes on base64
  ...
  Scenario Outline: Signature Verification
    ...
    And a <message>
    When I calculate SHA256 of the message
    And I decode <given_sign> with base64
    And convert the signature from byte to long
    And verify the signature with message hash 
  ...
  Scenario Outline: Encrypt
    ...
    Then the encoded message should be equal to <given_encrypted_message>

    Examples:
    | message  | given_encrypted_message |
  ...
  Scenario Outline: Encrypt
    ...
    -----END RSA PRIVATE KEY-----
    """
    And a <given_encrypted_message>
    When decode the message with base64
    And decrypt the result with RSA
    Then the decryped message should be equal to <given_decrypted_message>

    Examples:
    | given_decrypted_message | given_encrypted_message |
    | Test 1                  |  |
