...
def validate_registration(message_body):
  ...
  try:
    ...
    #Parse message
    invite_message_content = json.loads(invite_message_content_raw.decode())
    invite_message_body_content = json.loads(invite_message_content.get('messageBody'))
  except Exception as e:
  ...
  given_invitite_name = Teleferic_Identity.decrypt_content(given_invitite_name_raw)
...
def validate_invite(message_body):
  ...
  service_announcement_message = message_body.get('serviceAnnouncementMessage')
